...
<head>
	...
	<title>${message("member.memberDeposit.log")}[#if showPowered] - Powered By JFinalShop[/#if]</title>
		...
		<%_.each(depositLogs, function(depositLog, i) {%>
			<div class="list-group list-group-flat small">
				<div class="list-group-item">
					...
					<span title="<%-moment(depositLog.createdDate).format('YYYY-MM-DD HH:mm:ss')%>"><%-moment(depositLog.createdDate).format('YYYY-MM-DD')%></span>
				</div>
				<div class="list-group-item">
	...
	$().ready(function() {
		...
		$depositLogItems.infiniteScroll({
			url: function(pageNumber) {
				return "${base}/member/deposit/m_log?pageNumber=" + pageNumber;
			},
			pageSize: 10,

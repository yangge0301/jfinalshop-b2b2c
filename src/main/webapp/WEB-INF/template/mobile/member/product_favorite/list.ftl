...
<head>
	...
	<title>${message("member.productFavorite.list")}[#if showPowered] - Powered By JFinalShop[/#if]</title>
		...
		$().ready(function() {
			...
			$productFavoriteItems.infiniteScroll({
				url: function(pageNumber) {
					return "${base}/member/product_favorite/m_list?pageNumber=" + pageNumber;
				},
				pageSize: 10,

...
<head>
	...
	<title>${message("member.message.draft")}[#if showPowered] - Powered By JFinalShop[/#if]</title>
		...
		<%_.each(messages, function(message, i) {%>
			<div class="panel panel-flat small">
				<div class="panel-heading">
					...
					<span title="<%-moment(message.createdDate).format('YYYY-MM-DD HH:mm:ss')%>"><%-moment(message.createdDate).format('YYYY-MM-DD')%></span>
				</div>
		...
		$().ready(function() {
			...
			$messageItems.infiniteScroll({
				url: function(pageNumber) {
					return "${base}/member/message/m_draft?pageNumber=" + pageNumber;
				},
				pageSize: 10,

...
<head>
	...
	<title>${message("member.pointLog.list")}[#if showPowered] - Powered By JFinalShop[/#if]</title>
		...
		<%_.each(pointLogs, function(pointLog, i) {%>
			<div class="list-group list-group-flat small">
				<div class="list-group-item">
					...
					<span title="<%-moment(pointLog.createdDate).format('YYYY-MM-DD HH:mm:ss')%>"><%-moment(pointLog.createdDate).format('YYYY-MM-DD')%></span>
				</div>
				<div class="list-group-item">
	...
	$().ready(function() {
		...
		$pointLogItems.infiniteScroll({
			url: function(pageNumber) {
				return "${base}/member/point_log/m_list?pageNumber=" + pageNumber;
			},
			pageSize: 10,

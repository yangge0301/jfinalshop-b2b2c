...
<head>
	...
	<title>${message("member.order.view")}[#if showPowered] - Powered By JFinalShop[/#if]</title>
...
<body class="profile">
		...
		<div class="container-fluid">
			<div class="list-group list-group-flat">
				<div class="list-group-item">
					...
					[#else]
						<em class="pull-right orange">${message("Order.Status." + order.statusName)}</em>
					[/#if]
				</div>
			</div>
						...
						<div class="media">
							...
							<div class="media-body media-middle">
								...
								[#if orderItem.specificationsConverter?has_content]
									<span class="small gray-darker">${orderItem.specificationsConverter?join(", ")}</span>
								[/#if]
								[#if orderItem.typeName != "general"]
									<strong class="small red">[${message("Product.Type." + orderItem.typeName)}]</strong>
								[/#if]
							</div>
	...
	[#if (order.paymentMethod?? && order.amountPayable > 0) || (!order.hasExpired() && (order.statusName == "pendingPayment" || order.statusName == "pendingReview")) || (!order.hasExpired() && order.statusName == "shipped")]

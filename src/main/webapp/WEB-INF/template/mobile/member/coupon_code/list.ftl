...
<head>
	...
	<title>${message("member.couponCode.list")}[#if showPowered] - Powered By JFinalShop[/#if]</title>
					...
					<div class="list-group list-group-flat small">
						...
						<div class="list-group-item">
							...
							<%if (couponCode.usedDate) {%>
								<span class="pull-right" title="<%-moment(couponCode.usedDate).format('YYYY-MM-DD HH:mm:ss')%>"><%-moment(couponCode.usedDate).format('YYYY-MM-DD')%></span>
							<%} else {%>
						...
						<div class="list-group-item">
							...
							<%if (couponCode.coupon.endDate) {%>
								<span class="pull-right" title="<%-moment(couponCode.coupon.endDate).format('YYYY-MM-DD HH:mm:ss')%>"><%-moment(couponCode.coupon.endDate).format('YYYY-MM-DD')%></span>
							<%} else {%>
								<span class="pull-right">-</span>
							<%}%>
						</div>
		...
		$().ready(function() {
			...
			$couponCodeItems.infiniteScroll({
				url: function(pageNumber) {
					return "${base}/member/coupon_code/m_list?pageNumber=" + pageNumber;
				},
				pageSize: 10,

...
<head>
	...
	<title>${message("business.fullReductionPromotion.buy")} - Powered By JFinalShop</title>
		...
		[#if currentStore.endDate??]
			...
			$fullReductionPromotionForm.validate({
				...
				submitHandler: function(form) {
					$.ajax({
						url: "${base}/business/full_reduction_promotion/save_buy",
						type: "POST",
						data: {
							months: $months.val(),
							useBalance: $useBalance.prop("checked")
			...
			<div class="row">
				<div class="col-xs-12">
					<form id="fullReductionPromotionForm" class="form-horizontal" action="${base}/payment" method="post" target="_blank">
						...
						<input id="svcSn" name="paymentItems[0].svcSn" type="hidden">
						<div class="box">
							<div class="box-body">
								[#if currentStore.endDate??]

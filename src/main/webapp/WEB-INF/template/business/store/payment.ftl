...
<head>
	...
	<title>${message("business.store.payment")} - Powered By JFinalShop</title>
	...
	$().ready(function() {
		...
		[#if currentStore.statusName == "approved"]
			// 检查店铺状态
				...
				$.ajax({
					...
					success: function(data) {
						if (data.status != "approved") {
							location.href = "${base}/business/index/main";
						}
					}
				});
			}, 10000);
					...
					success: function(data) {
						if (data.platformSvcSn != null || (data.bail != null && data.bail > 0)) {
							...
							if (data.platformSvcSn != null) {
								...
								$('<input name="paymentItems[' + i + '].svcSn" type="hidden">').appendTo($storeForm).val(data.platformSvcSn);
								i++;
							}
							if (data.bail != null && data.bail > 0) {
								$('<input name="paymentItemList[' + i + '].type" type="hidden">').appendTo($storeForm).val("BAIL_PAYMENT");
								$('<input name="paymentItems[' + i + '].amount" type="hidden">').appendTo($storeForm).val(data.bail);
							}
							form.submit();
						} else {
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<a class="btn btn-primary" href="${base}/business/index/main">${message("business.paymentModal.paid")}</a>
								...
								<div class="row">
									<div class="col-xs-6">
										<dl class="items dl-horizontal clearfix">
											...
											[#if currentStore.statusName == "success"]
							...
							<div class="box-footer">
								<div class="row">
									<div class="col-xs-4 col-xs-offset-2">
										[#if currentStore.endDate?? || currentStore.statusName != "success"]

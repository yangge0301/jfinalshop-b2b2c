...
<head>
	...
	<title>${message("business.product.edit")} - Powered By JFinalShop</title>
	...
	<!-- 条码模板 -->
	<script id="addBarcodeTemplate" type="text/template">
		<div class="item" data-parameter-index="<%-barcodeIndex%>">
			<div class="form-group">
				<div class="col-xs-1 col-xs-offset-1 text-right">
					<p class="form-control-static">条码[<%-barcodeIndex%>]:</p>
				</div>
				<div class="col-xs-4">
					<input name="skuBarcode[<%-barcodeIndex%>].barcode" class="parameter-group form-control" type="text" maxlength="200">
				</div>
				<div class="col-xs-6">
	...
	<script id="addParameterGroupTemplate" type="text/template">
		<div class="item" data-parameter-index="<%-parameterIndex%>">
			<div class="form-group">
				...
				<div class="col-xs-4">
					<input name="parameterValueList[<%-parameterIndex%>].group" class="parameter-group form-control" type="text" maxlength="200">
				</div>
				<div class="col-xs-6">
					<p class="form-control-static">
						<a class="remove group" href="javascript:;">[${message("business.common.delete")}]</a>
		...
		<%_.each(parameters, function(parameter, i) {%>
			<div class="item" data-parameter-index="<%-parameterIndex%>">
				<div class="form-group">
					...
					<div class="col-xs-4">
						<input name="parameterValueList[<%-parameterIndex%>].group" class="parameter-group form-control" type="text" value="<%-parameter.group%>" maxlength="200">
					</div>
					<div class="col-xs-6">
						<p class="form-control-static">
							<a class="remove group" href="javascript:;">[${message("business.common.delete")}]</a>
	...
	<script id="specificationTemplate" type="text/template">
		<%_.each(specifications, function(specification, i) {%>
			<div class="specification-item form-group">
				<div class="col-xs-2">
					<input name="specificationItemList[<%-i%>].name" class="specification-item-name form-control text-right" type="text" value="<%-specification.name%>" data-value=<%-specification.name%>>
				</div>
				<div class="col-xs-10">
					<div class="row">
					...
					<tr data-ids="<%-ids.join(",")%>" <%if (initSkuValue != null) {%>title="${message("Sku.sn")}: <%-initSkuValue.sn%>"<%}%>>
						...
						<%_.each(entries, function(entry, j) {%>
							<td>
								...
								<input name="skuLists[<%-i%>].specificationValues[<%-j%>].id" type="hidden" value="<%-entry.id%>">
								<input name="skuLists[<%-i%>].specificationValues[<%-j%>].value" type="hidden" value="<%-entry.value%>">
							</td>
						<%})%>
						[#if product.typeName == "general"]
							<td>
						...
						<td>
							<input name="skuList[<%-i%>].market_price" class="market-price form-control" type="text" value="<%-marketPrice%>" maxlength="16"<%-!isEnabled ? " disabled" : ""%>>
						</td>
						[#if product.typeName == "general"]
							<td>
								<input name="skuList[<%-i%>].reward_point" class="reward-point form-control" type="text" value="<%-rewardPoint%>" maxlength="9"<%-!isEnabled ? " disabled" : ""%>>
							</td>
						[/#if]
						[#if product.typeName == "exchange"]
							<td>
								<input name="skuList[<%-i%>].exchange_point" class="exchange-point form-control" type="text" value="<%-exchangePoint%>" maxlength="9"<%-!isEnabled ? " disabled" : ""%>>
							</td>
						[/#if]
						<td>
						...
						<td>
							<input name="skuList[<%-i%>].is_default" class="is-default" type="checkbox" value="true"<%-isDefault ? " checked" : ""%><%-!isEnabled ? " disabled" : ""%>>
							<input name="_skuList[<%-i%>].is_default" type="hidden" value="false">
						</td>
						<td>
	...
	<script type="text/javascript">
		$().ready(function() {
			...
			<!-- 条码模板 -->
			var $barcode = $("#barcode");
			var $addBarcodeButton = $("#addBarcodeButton");
			...
			var addBarcodeTemplate = _.template($("#addBarcodeTemplate").html());
			...
			productImages = [
				[#list product.productImagesConverter as productImage]
					{
			...
			$addBarcodeButton.click(function() {
				var barcodeIndex = $barcodeContent.children(".item").size();
				$barcodeContent.append(addBarcodeTemplate({
					barcodeIndex: barcodeIndex
				}));
			});
			
			// 删除条码
			$barcodeContent.on("click", "a.remove", function() {
				var $element = $(this);
			...
			<div class="row">
				<div class="col-xs-12">
					<form id="productForm" class="form-horizontal" action="${base}/business/product/update" method="post">
						...
						<input id="sku.is_default" name="sku.is_default" type="hidden" value="true">
						<div class="box">
							<div class="box-body">
								...
								<div class="tab-content">
									<div id="base" class="tab-pane active">
										...
										<div class="form-group">
											...
											<div class="col-xs-4">
												<p class="form-control-static">${message("Product.Type." + product.typeName)}</p>
											</div>
										</div>
										<div class="form-group">
										...
										<div class="form-group">
											...
											<div class="col-xs-4">
												<input id="name" name="product.name" class="form-control" type="text" value="${product.name}" maxlength="200">
											</div>
										</div>
										<div class="form-group">
											<label class="col-xs-2 control-label" for="caption">${message("Product.caption")}:</label>
											<div class="col-xs-4">
												<input id="caption" name="product.caption" class="form-control" type="text" value="${product.caption}" maxlength="200">
											</div>
										</div>
										[#if product.typeName == "general"]
										...
										<div class="form-group[#if product.hasSpecification()] hidden[/#if]">
											...
											<div class="col-xs-4">
												<input id="marketPrice" name="sku.market_price" class="form-control" type="text" value="${product.defaultSku.marketPrice}" maxlength="16" title="${message("business.product.marketPriceTitle")}"[#if product.hasSpecification()] disabled[/#if]>
											</div>
										</div>
										<div class="form-group">
											<label class="col-xs-2 control-label">${message("Product.image")}:</label>
											<div class="col-xs-4">
												<input name="product.image" type="hidden" value="${product.image}" data-provide="fileinput" data-file-type="image">
											</div>
										</div>
										<div class="form-group">
											<label class="col-xs-2 control-label" for="unit">${message("Product.unit")}:</label>
											<div class="col-xs-4">
												<input id="unit" name="product.unit" class="form-control" type="text" value="${product.unit}" maxlength="200">
											</div>
										</div>
										<div class="form-group">
											<label class="col-xs-2 control-label" for="weight">${message("Product.weight")}:</label>
											<div class="col-xs-4">
												<input id="weight" name="product.weight" class="form-control" type="text" value="${product.weight}" maxlength="9" title="${message("business.product.weightTitle")}">
											</div>
										</div>
										[#if product.typeName == "general"]
											<div class="form-group[#if product.hasSpecification()] hidden[/#if]">
												...
												<div class="col-xs-4">
													<input id="rewardPoint" name="sku.reward_point" class="form-control" type="text" value="${product.defaultSku.rewardPoint}" maxlength="9" title="${message("business.product.rewardPointTitle")}"[#if product.hasSpecification()] disabled[/#if]>
												</div>
											</div>
										[/#if]
										[#if product.typeName == "exchange"]
											<div class="form-group[#if product.hasSpecification()] hidden[/#if]">
												...
												<div class="col-xs-4">
													<input id="exchangePoint" name="sku.exchange_point" class="form-control" type="text" value="${product.defaultSku.exchangePoint}" maxlength="9"[#if product.hasSpecification()] disabled[/#if]>
												</div>
											</div>
										[/#if]
										[#if product.hasSpecification()]
										...
										[#if product.typeName == "general" && promotions?has_content]
											<div class="form-group">
										...
										<div class="form-group">
											...
											<div class="col-xs-10 checkbox">
												<input id="isMarketable" name="product.is_marketable" type="text" value="${product.isMarketable?string("true", "false")}" data-toggle="checkbox-x">
												<label for="isMarketable" class="cbx-label">${message("Product.isMarketable")}</label>
												<input id="isList" name="product.is_list" type="text" value="${product.isList?string("true", "false")}" data-toggle="checkbox-x">
												<label for="isList" class="cbx-label">${message("Product.isList")}</label>
												<input id="isTop" name="product.is_top" type="text" value="${product.isTop?string("true", "false")}" data-toggle="checkbox-x">
												<label for="isTop" class="cbx-label">${message("Product.isTop")}</label>
												<input id="isDelivery" name="product.is_delivery" type="text" value="${product.isDelivery?string("true", "false")}" data-toggle="checkbox-x">
										...
										<div class="form-group">
											...
											<div class="col-xs-4">
												<input id="memo" name="product.memo" class="form-control" type="text" value="${product.memo}" maxlength="200">
											</div>
										</div>
										<div class="form-group">
											<label class="col-xs-2 control-label" for="keyword">${message("Product.keyword")}:</label>
											<div class="col-xs-4">
												<input id="keyword" name="product.keyword" class="form-control" type="text" value="${product.keyword}" maxlength="200" title="${message("business.product.keywordTitle")}">
											</div>
										</div>
									</div>
									<div id="introduction" class="tab-pane">
										<textarea name="product.introduction" data-provide="editor">${product.introduction}</textarea>
									</div>
											...
											[#list product.parameterValuesConverter as parameterValue]
												<div class="item" data-parameter-index="${parameterValue_index}" data-parameter-entry-index="${parameterValue.entries?size}">
													<div class="form-group">
														...
														<div class="col-xs-4">
															<input name="parameterValueList[${parameterValue_index}].group" class="parameter-group form-control" type="text" value="${parameterValue.group}" maxlength="200">
														</div>
														<div class="col-xs-6">
															<p class="form-control-static">
																<a class="remove group" href="javascript:;">[${message("business.common.delete")}]</a>
										...
										[#list product.productCategory.attributes as attribute]
											<div class="form-group">
												...
												<div class="col-xs-4">
													<select name="attribute_${attribute.id}" class="form-control">
														<option value="">${message("business.common.choose")}</option>
														[#list attribute.optionsConverter as option]
									...
									<div id="specification" class="tab-pane">
										...
										<div id="specificationContent">
											[#list product.specificationItemsConverter as specificationItem]
												<div class="specification-item form-group">
													<div class="col-xs-2">
														<input name="specificationItemList[${specificationItem_index}].name" class="specification-item-name form-control text-right" type="text" value="${specificationItem.name}" data-value="${specificationItem.name}">
													</div>
													<div class="col-xs-10">
														<div class="row">
									...
									<div id="barcode" class="tab-pane">
										<div class="form-group">
											<div class="col-xs-10 col-xs-offset-2">
												<button id="addBarcodeButton" class="btn btn-default" type="button">
													<i class="fa fa-plus"></i>
													增加条码
												</button>
											</div>
										...
										<div id="barcodeContent">
											[#list product.defaultSku.skuBarcodes as skuBarcode]
												<div class="item" data-parameter-index="<%-barcodeIndex%>">
													<div class="form-group">
														<div class="col-xs-1 col-xs-offset-1 text-right">
															<p class="form-control-static">条码[${skuBarcode_index}]:</p>
														</div>
														<div class="col-xs-4">
															<input name="skuBarcode[${skuBarcode_index}].id" type="hidden" value="${skuBarcode.id}">
															<input name="skuBarcode[${skuBarcode_index}].barcode" value="${skuBarcode.barcode}" class="parameter-group form-control" type="text" maxlength="200">
														</div>
														<div class="col-xs-6">
															<p class="form-control-static">
																<a class="remove group" href="javascript:;">[${message("business.common.delete")}]</a>

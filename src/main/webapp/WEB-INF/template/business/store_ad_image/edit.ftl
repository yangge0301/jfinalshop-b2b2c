...
<head>
	...
	<title>${message("business.storeAdImage.edit")} - Powered By JFinalShop</title>
	...
	$().ready(function() {
		...
		$storeAdImageForm.validate({
			rules: {
				...
				"storeAdImage.image": "required",
				"storeAdImage.orders": "digits"
			}
		});
			...
			<div class="row">
				<div class="col-xs-12">
					<form id="storeAdImageForm" class="form-horizontal" action="${base}/business/store_ad_image/update" method="post">
						<input name="storeAdImage.id" type="hidden" value="${storeAdImage.id}">
						<div class="box">
							<div class="box-body">
								<div class="form-group">
									<label class="col-xs-2 control-label" for="title">${message("StoreAdImage.title")}:</label>
									<div class="col-xs-4">
										<input id="title" name="storeAdImage.title" class="form-control" type="text" value="${storeAdImage.title}" maxlength="200">
									</div>
								</div>
								<div class="form-group">
									<label class="col-xs-2 control-label" for="url">${message("StoreAdImage.url")}:</label>
									<div class="col-xs-4">
										<input id="url" name="storeAdImage.url" class="form-control" type="text" value="${storeAdImage.url}" maxlength="200">
									</div>
								</div>
								<div class="form-group">
									<label class="col-xs-2 control-label item-required">${message("StoreAdImage.image")}:</label>
									<div class="col-xs-4">
										<input name="storeAdImage.image" type="hidden" value="${storeAdImage.image}" data-provide="fileinput" data-file-type="image">
									</div>
								</div>
								<div class="form-group">
									<label class="col-xs-2 control-label" for="order">${message("business.common.order")}:</label>
									<div class="col-xs-4">
										<input id="storeAdImage.orders" name="order" class="form-control" type="text" value="${storeAdImage.orders}" maxlength="9">
									</div>
								</div>
							</div>

...
<head>
	...
	<title>${message("business.instantMessage.edit")} - Powered By JFinalShop</title>
	...
	$().ready(function() {
		...
		$instantMessageForm.validate({
			rules: {
				"instantMessage.name": "required",
				type: "required",
				"instantMessage.account": "required",
				"instantMessage.orders": "digits"
			}
		});
			...
			<div class="row">
				<div class="col-xs-12">
					<form id="instantMessageForm" class="form-horizontal" action="${base}/business/instant_message/update" method="post">
						<input name="instantMessage.id" type="hidden" value="${instantMessage.id}">
						<div class="box">
							<div class="box-body">
								<div class="form-group">
									<label class="col-xs-2 control-label item-required" for="name">${message("InstantMessage.name")}:</label>
									<div class="col-xs-4">
										<input id="name" name="instantMessage.name" class="form-control" type="text" value="${instantMessage.name}" maxlength="200">
									</div>
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									...
									<div class="col-xs-4">
										<input id="account" name="instantMessage.account" class="form-control" type="text" value="${instantMessage.account}" maxlength="10">
									</div>
								</div>
								<div class="form-group">
									<label class="col-xs-2 control-label" for="order">${message("business.common.order")}:</label>
									<div class="col-xs-4">
										<input id="order" name="instantMessage.orders" class="form-control" type="text" value="${instantMessage.orders}" maxlength="9">
									</div>
								</div>
							</div>

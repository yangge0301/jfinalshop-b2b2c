...
<head>
	...
	<title>${message("business.storeProductCategory.edit")} - Powered By JFinalShop</title>
	...
	$().ready(function() {
		...
		$storeProductCategoryForm.validate({
			rules: {
				"storeProductCategory.name": "required",
				"storeProductCategory.orders": "digits"
			}
		});
			...
			<div class="row">
				<div class="col-xs-12">
					<form id="storeProductCategoryForm" class="form-horizontal" action="${base}/business/store_product_category/update" method="post">
						<input name="storeProductCategory.id" type="hidden" value="${storeProductCategory.id}">
						<div class="box">
							<div class="box-body">
								<div class="form-group">
									<label class="col-xs-2 control-label item-required" for="name">${message("StoreProductCategory.name")}:</label>
									<div class="col-xs-4">
										<input id="name" name="storeProductCategory.name" class="form-control" type="text" value="${storeProductCategory.name}" maxlength="200">
									</div>
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									...
									<div class="col-xs-4">
										<input id="order" name="storeProductCategory.orders" class="form-control" type="text" value="${storeProductCategory.orders}" maxlength="9">
									</div>
								</div>
							</div>

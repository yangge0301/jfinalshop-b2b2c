...
import com.jfinalshop.shiro.core.handler.AuthenticatedAuthzHandler;
import com.jfinalshop.shiro.core.handler.AuthzHandler;
import com.jfinalshop.shiro.core.handler.CompositeAuthzHandler;
import com.jfinalshop.shiro.core.handler.GuestAuthzHandler;
import com.jfinalshop.shiro.core.handler.PermissionAuthzHandler;
import com.jfinalshop.shiro.core.handler.RoleAuthzHandler;
import com.jfinalshop.shiro.core.handler.UserAuthzHandler;
...
public class ShiroPlugin implements IPlugin {
	...
	public boolean start() {
		...
		for (Route route : routes.getRouteItemList()) {
			...
			List<Annotation> controllerAnnotations = getAuthzAnnotations(route.getControllerClass());
			
			// 逐个遍历方法。
			//Method[] methods = controllerClass.getMethods();
			Method[] methods = route.getControllerClass().getMethods();
			for (Method method : methods) {
				...
				if (!excludedMethodName.contains(method.getName()) && method.getParameterTypes().length == 0) {
					...
					if (authzHandler != null) {
						...
						String actionKey = createActionKey(route.getControllerClass(), method, controllerKey);

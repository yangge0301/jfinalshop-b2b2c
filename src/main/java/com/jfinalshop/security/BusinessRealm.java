...
public class BusinessRealm extends AuthorizingRealm {
	...
	private BusinessService businessService = HasorUtils.getBean(BusinessService.class);
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		if (business != null && business.getIsEnabled()) {
			SimpleAuthenticationInfo info = new SimpleAuthenticationInfo(business, business.getPassword(), getName());
			return info;
		} else {
			return null;
		}
	 ...
	 * 多realm该方法会在多个realm之间调用，所以需要判断当前用户是否是当前类中的Business对象
	 * @param principals
	 *            用户信息
	 * @return
	 */
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		// 判断是否是当前用户登录的对象  
        if (principals == null || !(principals.getPrimaryPrincipal() instanceof Business)) {  
            return null;  
        }  
		String loginName = ((Business) principals.fromRealm(getName()).iterator().next()).getUsername();
		SimpleAuthorizationInfo info = new SimpleAuthorizationInfo();
		Set<String> roleSet = new LinkedHashSet<String>(); // 角色集合
		Set<String> permissionSet = new LinkedHashSet<String>(); // 权限集合
		List<Role> roles = null;
		Business business = businessService.findByUsername(loginName);
		if (business != null) {
			// 遍历角色
			roles = roleService.findByBusiness(business);
		} else {

...
public class MemberRealm extends AuthorizingRealm {
	...
	private MemberService memberService = HasorUtils.getBean(MemberService.class);
	 ...
	 * 多realm该方法会在多个realm之间调用，所以需要判断当前用户是否是当前类中的Member对象
	 * @param principals
	 *            用户信息
	 * @return
	 */
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		// 判断是否是当前用户登录的对象  
        if (principals == null || !(principals.getPrimaryPrincipal() instanceof Member)) {  
            return null;  
        }  
		String loginName = ((Member) principals.fromRealm(getName()).iterator().next()).getUsername();
		SimpleAuthorizationInfo info = new SimpleAuthorizationInfo();
		Set<String> roleSet = new LinkedHashSet<String>(); // 角色集合
		Set<String> permissionSet = new LinkedHashSet<String>(); // 权限集合
		List<Role> roles = null;
		Member member = memberService.findByUsername(loginName);
		if (member != null) {
			// 遍历角色
			roles = roleService.findByMember(member);
		} else {

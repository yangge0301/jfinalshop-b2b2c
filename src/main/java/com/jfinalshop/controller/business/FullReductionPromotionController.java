 ...
 * 
 */
@ControllerBind(controllerKey = "/business/full_reduction_promotion")
public class FullReductionPromotionController extends BaseController {
	 ...
	 * 到期日期
	 */
	@ActionKey("/business/full_reduction_promotion/end_date")
	public void endDate() {
		Store currentStore = businessService.getCurrentStore();
		Map<String, Object> data = new HashMap<>();
	...
	public void buy() {
		...
		if (currentStore.getTypeName().equals(Store.Type.self) && currentStore.getFullReductionPromotionEndDate() == null) {
			redirect("list");
		}
		PromotionPlugin promotionPlugin = pluginService.getPromotionPlugin(FullReductionPromotionPlugin.ID);
		if (promotionPlugin == null || !promotionPlugin.getIsEnabled()) {
			setAttr("errorMessage", "插件未启用!");
		...
		render("/business/full_reduction_promotion/buy.ftl");
	}

	/**
	 * 购买
	 */
	@ActionKey("/business/full_reduction_promotion/save_buy")
	public void saveBuy() {
		Integer months = getParaToInt("months");
		Boolean useBalance = getParaToBoolean("useBalance");
		Store currentStore = businessService.getCurrentStore();
		...
		if (amount.compareTo(BigDecimal.ZERO) > 0) {
			if (BooleanUtils.isTrue(useBalance)) {
				if (currentUser.getBalance().compareTo(amount) < 0) {
					Results.unprocessableEntity(getResponse(), "business.fullReductionPromotion.insufficientBalance");
					return;
				}
	 ...
	 * 赠品选择
	 */
	@ActionKey("/business/full_reduction_promotion/gift_select")
	...
	public void add() {
		...
		render("/business/full_reduction_promotion/add.ftl");
	}
	...
	public void save() {
		...
		promotion.setType(Promotion.Type.fullReduction.ordinal());
	...
	public void edit() {
		...
		setAttr("memberRanks", memberRankService.findAll());
		setAttr("coupons", couponService.findList(currentStore));
		render("/business/full_reduction_promotion/edit.ftl");
	}
	...
	public void list() {
		...
		setAttr("page", promotionService.findPage(currentStore, Promotion.Type.fullReduction, pageable));
		render("/business/full_reduction_promotion/list.ftl");
	}
